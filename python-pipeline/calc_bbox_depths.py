...
def generate_bb_pixel_mapping(pixel_bb_mapping: Dict[Point, List[BBox]]) -> Dict[Tuple[BBox], List[Point]]:
    '''
...
def resolve_overlapping_points(bb_pixel_mapping: Dict[Tuple[BBox], List[Point]], depths) -> Dict[Tuple[BBox], List[Point]]:
    ...
    key_order = sorted(bb_pixel_mapping, key=lambda x: len(x)) # These are all the sorted keys
    ...
    for key in key_order:
        if len(key) == 1:
            ...
            min_median: Tuple[BBox, float] = min([((i,), median_depths[(i, )]) for i in key], key=lambda x:x[1])
...
def calculate_bbox_depths(bbs: List[BBox], depths: np.ndarray) -> List[Tuple[BBox, float]]:
    '''
    ...
    # bounding box
    result = [(bbox, calculate_median(deconflicted_bb_pixel_mapping[bbox], depths))
